...
def h_cluster_multi(organized_ls, input_folder, scheme, stimulus_ls, threshold,n_pc_ls, norm=0):
    ...
    #     return labels
    def plot_heat_dendro(all_trace_np, plot_trace, genotype_index, n_pc, range, axes):
        assert len(axes) == len(stimulus_ls) + 3
        if n_pc == -1:
            n_pc == np.min(all_trace_np.shape)
        pca = decomposition.PCA(n_components=n_pc, random_state=0)
        kept_pc = pca.fit_transform(all_trace_np)
        ...
        frame_count_index = 0
        for ind, ax in enumerate(axes[:len(stimulus_ls)]):
            sti_len = (scheme[scheme['stimulation'] == stimulus_ls[ind]]['video_end'] - scheme[scheme['stimulation'] == stimulus_ls[ind]]['video_start'] + 1).values[0]
            axes[ind].imshow(plot_trace[dendro_label, frame_count_index:frame_count_index+sti_len], cmap='jet', vmin=range[0], vmax=range[1], aspect='auto',
                             interpolation='none')
            axes[ind].axvline(x = (scheme[scheme['stimulation'] == stimulus_ls[ind]]['stimulus_start'] - scheme[scheme['stimulation'] == stimulus_ls[ind]]['video_start'] + 1).values[0],ymin=0,ymax=1,color='white', linestyle='dashed', linewidth=1)
            axes[ind].axvline(x=(scheme[scheme['stimulation'] == stimulus_ls[ind]]['stimulus_end'] -
                                 scheme[scheme['stimulation'] == stimulus_ls[ind]]['video_start'] + 1).values[0],
                              ymin=0, ymax=1, color='white', linestyle='dashed', linewidth=1)
            axes[ind].title.set_text(stimulus_ls[ind])
            frame_count_index += sti_len
        color_ls = ['#808080', '#000000']

        scale = [0,0.5,1]
        print(genotype_index[dendro_label])
        genotype_index = np.unique(genotype_index, return_inverse=True)[1]
        # for ind, geno in enumerate(np.unique(genotype_index)):
        #     geno_cmap.update({geno: color_ls[ind]})

        geno_cmap = matplotlib.colors.ListedColormap(color_ls)
        norm = matplotlib.colors.BoundaryNorm(scale, len(color_ls))
        print(genotype_index[dendro_label])
        axes[-2].imshow(genotype_index[dendro_label].reshape(-1,1), cmap=geno_cmap,norm=norm, aspect='auto',
                       interpolation='none')
        axes[-3].plot([10, 15],[0.5,0.5], color='k', lw=5,  solid_capstyle="butt")
        axes[-3].set_xlim([0, sti_len])
    ...
    stimulus_ls.remove('MoSug')
    stimulus_ls.remove('Mix')
    ...
    for n_pc in n_pc_ls:
        ...
        fig = plt.figure(figsize=(10, 6))
        gs_ls = gridspec.GridSpec(nrows=1, ncols=len(stimulus_ls)+3)#, width_ratios=[4, 1], wspace=wspace)
        for gs in gs_ls:
            axes.append(plt.subplot(gs))

        labels = plot_heat_dendro(norm_all_trace_np, plot_trace=all_trace_np, n_pc=n_pc, range=[-0.5, 5],
                         axes=axes, genotype_index=genotype_index)
        ...
        for genotype in organized['genotype'].unique():
            ...
            plot_cluster_trace_grid_new(output_folder, plot_org, scheme, row_label, stimulus_ls, ylim=[-1, 7],
                                                 linewidth=3, color='k', height=10, width=10, svg=1, cluster_order=[],
                                                 stimulus_bar_y=-1,
                                                 stimulus_bar_lw=20, scalebar_frame_length=10,
