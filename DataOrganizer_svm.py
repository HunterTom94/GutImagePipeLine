...
def organized_gen(input_folder, raw=0, svm_model_name='', filter_ls=[], custom_peak=''):
    ...
    if svm_model_name != '':
        ...
        df = pd.DataFrame(sti_df[['ROI_index', 'stimulation']])
        df['pred'] = y_pred
        throw_df = df[df['pred'] == 0][['ROI_index', 'stimulation']]
        ...
        throw_df = throw_df.sort_values(by='ROI_index')

        temp_scheme = scheme.set_index('stimulation', drop=False)
        def replace_nan(df, ROI_index, stimulus):
            df = df.set_index('ROI_index', drop=False)
            try:
                df.at[ROI_index, temp_scheme.ix[stimulus]['video_start']-1:temp_scheme.ix[stimulus]['video_end']] = 'nan'
            except ValueError:
                pass
            df = df.reset_index(drop=True)
            return df

        # all_data = all_data[all_data['ROI_index'].isin(throw_df)]
        all_data = all_data.sort_values(by='ROI_index')
        for _, row in throw_df.iterrows():
            if 'b' in row['ROI_index'].split('_')[1]:
                continue
            all_data = replace_nan(all_data, row['ROI_index'], row['stimulation'])

        if raw:
            file_name = 'filtered_f.csv'
        else:
            file_name = 'filtered_dff.csv'
        all_data = all_data.sort_values(by='ROI_index')
        all_data.to_csv(input_folder + 'svm_filtered\\' + file_name, sep='\t')
        exit()
