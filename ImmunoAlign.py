...
folder = 'D:\\Gut Imaging\\Videos\\Temp\\d335758\\'
...
def align_video(stack_src, stack_dst):
    ...
    stack_src_max = np.clip(stack_src, 0, 10000).transpose()
...
def align_video_new(stack_src, stack_dst, anti):
    x_tracker = 0
    y_tracker = 0
    rot_tracker = 0
    ...
    stack_src_max = np.clip(stack_src, 0, 10000).transpose()
    stack_dst_max = np.clip(stack_dst, 0, 80).transpose()
    stack_dst_max[stack_dst_max < 40] = 0
    anti_max = np.clip(anti, 0, 10000).transpose()

    assert stack_src_max.shape == stack_dst_max.shape == anti_max.shape

    stack_src_max = cv2.normalize(stack_src_max, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8UC1)
    stack_dst_max = cv2.normalize(stack_dst_max, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8UC1)
    anti_max = cv2.normalize(anti_max, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8UC1)
    # zeros = cv2.normalize(np.zeros(stack_src_max.shape).astype(int), None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8UC1)
    # draw_pad = np.stack((anti_max, stack_src_max, stack_dst_max))
    draw_pad = np.stack((stack_src_max,stack_dst_max, anti_max))
    draw_pad = np.swapaxes(draw_pad, 0, 2)
    ...
    dummy = draw_pad.copy()
    src_dummy = draw_pad[:,:,0].copy()
    anti_dummy = draw_pad[:,:,2].copy()
    ...
    while k != 27:
        ...
        moved_anti = x_move(moved_anti, x_tracker)
        moved_anti = y_move(moved_anti, y_tracker)
        moved_anti = rotation(moved_anti, rot_tracker)

        draw_pad[:, :, 0] = moved_src
        draw_pad[:, :, 2] = moved_anti
...
immuno_image_ls = []
sample = read_tif(folder + folder.split('\\')[-2] + '.tif')
...
gcamp = read_tif(folder + 'Immuno_Image2_335758_antiG7c.tif')[0:max.shape[0], 0:max.shape[1]]
anti = read_tif(folder + 'Immuno_Image1_335758_antiTk.tif')[0:max.shape[0], 0:max.shape[1]]

aligned_immuno, merged = align_video_new(gcamp, max, anti)
exit()
