...
###################################################################################################
folder = 'D:\\Gut Imaging\\Videos\\Temp\\d246874\\'  ##
folder = 'D:\\Gut Imaging\\Videos\\ParaTest\\2_SARFIA_paraTest\\d24982504_f15t1.5\\'
segment_width = 30  ##
refresh_points = 0  ##
redraw = 0  ##
...
fine_tune_region = 0  ## 1: Manual region specification
    ...
    def fillSegment():
        ...
        for key_ind, key in enumerate(cumulative_AP):
            if cumulative_AP[key] <= 1:
                cv2.putText(draw_pad, text='{}: {}%'.format(key, np.round(cumulative_AP[key]*100, 1)), org=(750, 50 + key_ind*30), fontFace = cv2.FONT_HERSHEY_SIMPLEX, fontScale = 1, color=(0,0,255))
            else:
