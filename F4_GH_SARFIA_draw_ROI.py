...
folder = 'H:\\GutImagingData\\SARFIA_drawROI\\d24982504\\'
...
for param in params[::-1]:
    ...
    bin_rois[rois != 0] = 1
    drawing_pad = BGR_max.copy()
    contours, hierarchy = cv2.findContours(bin_rois, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
    cv2.drawContours(drawing_pad, contours, -1, tuple(int(color_dict[param][i:i+2], 16) for i in (0, 2, 4))[::-1], 1)
    pt2 = (400,232)
    cropped = drawing_pad[pt2[1] - 100:pt2[1], pt2[0] - 100:pt2[0]]
    cropped = cv2.resize(cropped, (int(cropped.shape[1] * 5), int(cropped.shape[0] * 5)))
    cv2.imwrite(folder + 'cropped_t_{}.png'.format(param), cropped)
