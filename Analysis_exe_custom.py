import pandas as pd
import numpy as np
from organized_Clustering_custom import UMAP_gen, UMAP_gen_paralle, raw_pca, h_cluster_multi
import itertools
from DataOrganizer_custom import organized_gen, data_plus_label, roiName2trace, parameters_mean_sem, parameters_mean_sem_cluster, organized2excel
...
def load_org_hc(org_name_ls, root_folder):
    org_ls = []
    for org_name in org_name_ls:
        temp_org = pd.read_pickle(root_folder + '{}\\{}_organized_data.pkl'.format(org_name, org_name))
        temp_org['genotype'] = org_name
        org_ls.append(temp_org)
    return org_ls
...
# input_folder = 'D:\\Gut Imaging\\Videos\\svm\\CCHa2\\'
# input_folder = 'D:\\Gut Imaging\\Videos\\Temp_UMAP\\Dh31_Dosage_Comb_1\\'
# input_folder = 'D:\\Gut Imaging\\Videos\\Temp_UMAP\\Pros_PhePro\\'
...
# # organized2excel(organized, input_folder,stimulus_ls, all_trace=1)
# raw_pca(organized, input_folder, stimulus_ls)
# exit()

# ###############################################     Database Generator       ###########################################
# sample_ls = ['312529','313337','314044','316671']
# custom_peak = ''
# filter_ls = []  # Exact Stimuli Names to be Excluded from Filter
# # svm_model_name = ''
# svm_model_name = 'Pros'
# raw_organized = organized_gen(sample_ls, scheme, input_folder, raw=1)
# organized = organized_gen(sample_ls, scheme, input_folder, raw=0, svm_model_name=svm_model_name, filter_ls=filter_ls, custom_peak=custom_peak)
# exit()

####################################     Hierarchical Clustering      ########################################
# # *********************************************************************************************
root_folder = 'D:\\Gut Imaging\\Videos\\Temp_UMAP\\'
scheme = pd.read_excel('D:\\Gut Imaging\\Videos\\Temp_UMAP\\Receptor_CG13575\\DeliverySchemes.xlsx')
org_name_ls = ['Receptor_CG13575', 'WT_CG13575']
threshold = 6
n_pc_ls = [13]#range(12, 15)
norm = 1
# # *********************************************************************************************
#
org_ls = load_org_hc(org_name_ls, root_folder)
stimulus_ls = scheme.iloc[:, 1].to_list()
h_cluster_multi(org_ls,root_folder, scheme, stimulus_ls, threshold=threshold,
                n_pc_ls=n_pc_ls, norm=norm)
exit()
...
screen = 1
num_screen = 0
# screened_paras = 'Screened Parameters_1.csv'  #
screened_paras = ''

if not screened_paras:
    ...
    # UMAP_nearest_neighbor_ls = [90,95,100,105]
    # UMAP_nearest_neighbor_ls = [95]
    # HDBSCAN_para_ls = list(itertools.permutations([3,5,8,10,12,15,18,20,25,28,30,40,50,70,90,120,150,200,300,400,500,600],2))
    # HDBSCAN_para_ls = list(itertools.permutations([3,5,10,15,20,25,30,50,100,120,150,200,300,400,500,600],2))
    HDBSCAN_para_ls = [[25,30]]  # cluster parameter
    # random_state_ls = [1,2,3,5,6,8,10,12,15,18,20,30,40,50,60,80,100,120,150,180,200,250,300,350,400,500,600,800,1000,1200]  # seed
    random_state_ls = [100]  # seed
    parallel_input_ls = list(itertools.product(UMAP_nearest_neighbor_ls, HDBSCAN_para_ls, random_state_ls))
else:
    screened_paras_df = pd.read_csv(input_folder + screened_paras)
    UMAP_nearest_neighbor_ls = screened_paras_df['UMAP_nearest_neighbor'].tolist()
    random_state_ls = screened_paras_df['UMAP_random_seed'].tolist()
    HDBSCAN_min_c_ls = screened_paras_df['HDBSCAN_min_cluster'].tolist()
    HDBSCAN_min_s_ls = screened_paras_df['HDBSCAN_min_sample'].tolist()
    parallel_input_ls = list(zip(UMAP_nearest_neighbor_ls, zip(HDBSCAN_min_c_ls, HDBSCAN_min_s_ls), random_state_ls))
peak_type = 'average'  # Choose from 'average', 'individual', 'region', 'fine_region', 'custom'
z_score_norm = 1
...
dynamics_dff_peak = 0
dynamics_dff_peak_time = 0
dynamics_dff_prime_peak = 0
dynamics_dff_prime_peak_time = 0

heatmap_range = (-2.5, 2.5)
...
region_palatte_dict = {'R1': [153, 93, 19, 255],
                       'R2': [234, 0, 1, 255],
                       'R3': [255, 141, 30, 255],
                       'R4': [30,32,255, 255],
                       'R5': [18, 68, 44, 255]}
...
def UMAP_exe(parallel_input):
    result_dict = UMAP_gen_paralle(output_folder, organized, stimulus_ls, hdbscan_para=parallel_input[1], n_n=parallel_input[0],
                     random_state=parallel_input[2], row_num=row_num, col_num=col_num, peak_type=peak_type,
                     ...
                     xlim_para=xlim_para, ylim_para=ylim_para, exclude_kcl=exclude_kcl,
                     UMAP_region_as_input=UMAP_region_as_input,
                     dynamics_dff_peak=dynamics_dff_peak, dynamics_dff_peak_time=dynamics_dff_peak_time,
                     dynamics_dff_prime_peak=dynamics_dff_prime_peak,
                     dynamics_dff_prime_peak_time=dynamics_dff_prime_peak_time, num_screen=num_screen)
    if num_screen:
        result_dict.update({'UMAP_nearest_neighbor': parallel_input[0], 'UMAP_random_seed': parallel_input[2],
                            'HDBSCAN_min_cluster': parallel_input[1][0], 'HDBSCAN_min_sample': parallel_input[1][1]})
        return result_dict



# parallel_input_ls = list(itertools.product(UMAP_nearest_neighbor_ls, HDBSCAN_para_ls, random_state_ls))
num_cores = np.min([mp.cpu_count(), len(parallel_input_ls)])
print('{} Parameter Combinations to Screen'.format(len(parallel_input_ls)))

if screen:
    if __name__ == '__main__':

        pool = mp.Pool(num_cores)
        results = pool.map(UMAP_exe, parallel_input_ls)

        pool.close()
        pool.join()

        if num_screen:
            pd.DataFrame(results).to_csv(input_folder + 'Screened Parameters.csv')

else:
    UMAP_gen(output_folder, organized, stimulus_ls, HDBSCAN_para_ls, UMAP_nearest_neighbor_ls, random_state_ls, row_num,
             col_num, peak_type=peak_type, z_score_norm=z_score_norm, svg=svg, hue_norm=heatmap_range,
             ...
             dot_edgewidth=dot_edgewidth, anyresp=anyresp, xlim_para=xlim_para, ylim_para=ylim_para,
             cluster_palatte_dict=cluster_palatte_dict, exclude_kcl=exclude_kcl,
             UMAP_region_as_input=UMAP_region_as_input, dynamics_dff_peak=dynamics_dff_peak, dynamics_dff_peak_time=dynamics_dff_peak_time,
             dynamics_dff_prime_peak=dynamics_dff_prime_peak, dynamics_dff_prime_peak_time=dynamics_dff_prime_peak_time)


#####################################    Cluster Mean SEM Parameters Generator       ########################################
# *********************************************************************************************
# roi_names_file = ''
# cluster_label_name = '115_20_10_2cluster_label.txt'
# # *********************************************************************************************
#
# organized = read_organized()
# organized = roi_filter(organized, roi_names_file)
...
# cluster_label_name = '15_clusters_95_25_30_100cluster_label.txt'
...
# height = 8
# width = 4
# cluster_order = [11,12,7,6,1,8,2,3,13,5,14,10,9,4]
# stimulus_bar_y= -1
# stimulus_bar_lw= 6
