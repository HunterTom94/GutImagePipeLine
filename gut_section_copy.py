...
###################################################################################################
folder = 'D:\\Yinan\\GutImageDataAnalysis\\d345354\\'  ##
# folder = 'D:\\Gut Imaging\\Videos\\Results\\NutrientSensing\\Receptors\\GPCR_v9363\\d316671\\' ##
segment_width = 30  ##
refresh_points = 0  ##
redraw = 0  ##
brightness_scale = 1.7  ##
segment_point_scale = [1, 1]
fine_tune_region = 1  ## 1: Manual region specification
fine = 1  ## 1: tune subregion manually; 0: subregion readjusted automatically
KCl_num = '3422'                                                                                ##
###################################################################################################
...
def gut_section_exe(folder, segment_width, refresh_points, redraw, brightness_scale, fine_tune_region, fine):
    ...
    if fine_tune_region:
        ...
        roi_df['distance_to_origin'] = np.round(roi_df['distance_to_origin'] * src_scale, 1)
...
def assign_segments(roi_df):
    ...
    # origin_section = np.min(roi_df['segment']).astype(int)
    origin_section = 0
...
def region_select(fine=0):
    ...
    def segment_update(folder, cumulative_AP):
        ...
        for key_ind in range(len(cumulative_AP)):
            ...
            segment_df.at[(segment_df['cum_AP'] >= start * 100) & (segment_df['cum_AP'] < end * 100), 'region'] = \
            list(cumulative_AP.keys())[key_ind]
        segment_df.at[segment_df['cum_AP'] >= 100, 'region'] = list(cumulative_AP.keys())[key_ind]
        segment_df.to_csv(folder + 'manual_segment.csv', index=False)


    # def seg2AP(a):
    #     AP_min = np.min(segment_df['distance_to_origin'].values)
    #     AP_max = np.max(segment_df['distance_to_origin'].values - AP_min)
    #
    #     print(AP_min)
    #     print(AP_max)
    #
    ...
    def seg2AP(a):
        mid_lines = pd.read_pickle(folder + 'segment_files\\mid_lines.pkl')
        AP_min = (mid_lines.iloc[1]['length'] * 0.5) * src_scale
        AP_max = (mid_lines.iloc[-1]['length'] * 0.5 + mid_lines.iloc[:-1]['length'].sum()) * src_scale
    ...
    temp_region_coord_holder = (1/src_scale) * temp_region_coord_holder[:,[1,0]]
    ...
    def fillSegment():
        ...
        for region_coord_index in range(temp_region_coord_holder.shape[0])[
            ...
            if isinstance(segment_index, int):
                distance = seg2AP((mid_lines.iloc[segment_index]['length'] * 0.5 + mid_lines.iloc[:segment_index]['length'].sum())*src_scale)
                cumulative_AP.update({list(cumulative_AP.keys())[dict_key_index[region_coord_index]]: distance/100})
        ...
        for key_ind, key in enumerate(cumulative_AP):
            if cumulative_AP[key] <= 1:
                cv2.putText(draw_pad, text='{}: {}%'.format(key, int(np.rint(cumulative_AP[key]*100))), org=(750, 50 + key_ind*30), fontFace = cv2.FONT_HERSHEY_SIMPLEX, fontScale = 1, color=(255, 255, 255))
            else:
                cv2.putText(draw_pad, text='{}: {}%'.format(key, int(np.floor(cumulative_AP[key] * 100))),
                            org=(750, 50 + key_ind * 30), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1,
                            color=(255, 255, 255))
    ...
    def region_select_callback(action, x, y, flags, userdata):
        ...
        def auto_append(x,y, region_start, region_ratio):
            ...
            clicked_length = seg2AP((mid_lines.iloc[segment_index]['length'] * 0.5 +
                               mid_lines.iloc[:segment_index]['length'].sum()) * src_scale)
    ...
    if os.path.isdir(KCl_dir):
        ...
        KCl_ROI_df = pd.read_csv(KCl_dir + 'manual_segment.csv')
        ROI_df = pd.read_csv(folder + 'manual_segment.csv')
        MC_ROI_df = pd.read_csv('D:\\Gut Imaging\\Videos\\CommonFiles\\f_file\\f_{}.txt'.format(folder.split('\\')[-2][1:]), sep='\t')
        MC_ROI_index = np.array(list(MC_ROI_df.columns)[1:]).astype(int).flatten()
        KCL_count = KCl_ROI_df.groupby(['region'])['ROI_ID'].count()
        MC_kept_ROI_df = ROI_df[~ROI_df['ROI_ID'].isin(MC_ROI_index)]
        if MC_kept_ROI_df.shape[0] == 0:
            MC_count = np.zeros((len(KCL_count),))
            count_df = pd.DataFrame(dict(KCl=KCL_count)).reset_index()
            count_df['MC_throw'] = MC_count
        else:
            MC_count = MC_kept_ROI_df.groupby(['region'])['ROI_ID'].count()
            count_df = pd.DataFrame(dict(KCl=KCL_count, MC_throw=MC_count)).reset_index()
        if count_df['MC_throw'].isnull().values.any():
        ...
        count_df.to_csv(folder + '\\region_count.csv')
    else:
