...
# sti_df.to_pickle('KCl_svm_GT.pkl')
sti_df = pd.read_pickle('KCl_svm_GT.pkl')
...
# plt.savefig('Z:\\Christina\\FromYinan\\KCl_GT_improvement\\F13d.svg')
plt.show()
print(means_hat)
print(sds_hat)
print(weights_hat)
...
ax = plt.subplot()
# embedding = np.concatenate((np.log(keep['AUC'].values).reshape(-1, 1), keep['drv_pearson'].values.reshape(-1, 1)),axis=1)
...
# # ax.scatter(np.log(sti_df['AUC'].values), sti_df['drv_pearson'].values, s=0.5)
# clusterer = hdbscan.HDBSCAN(min_cluster_size=300, min_samples=1)
...
sns.scatterplot(x='AUC', y='drv_pearson', data=clusterDf, hue='label', s=0.5, linewidth=0, ax=ax, alpha=0.3)
ax.plot(x,y, 'r--')
# plt.savefig('Z:\\Christina\\FromYinan\\KCl_GT_improvement\\F13k.svg')
# ax.scatter(embedding[:,0], embedding[:,1], s=0.5, alpha=0.3)
# c=0.5

plt.show()
# exit()

# data = np.multiply(sti_df['drv_pearson'].values, np.sqrt(sti_df['AUC'].values))
# data = np.log(data)
# ax1 = plt.subplot()
# bin_size = 0.1
# bins = np.arange(-8,5,bin_size)
# histogram(color='#ABEDD890', bin_size=bin_size)




# sti_df_below_zero = sti_df[sti_df['drv_pearson'] <= 0]
# sti_df_below_zero['label'] = 0
# sti_df_above_zero = sti_df[sti_df['drv_pearson'] > 0]
# feature_np = sti_df_above_zero[['peak_individual','drv_peak','pearson','drv_pearson']].values
# feature_np = sti_df[['drv_peak','pearson']].values
# feature_np = sti_df[['drv_peak','drv_pearson', 'AUC']].values
# feature_np = stats.zscore(feature_np, axis=0)
# pca = PCA(n_components=2)
# reduced_data = pca.fit_transform(feature_np)
# # ax = plt.subplot()
# # ax.scatter(reduced_data[:,0], reduced_data[:,1], s=0.5)
# # ax.scatter([-2.2,-0.7],[3, -1.2], s=2)
# # plt.show()
# # exit()
# principalDf = pd.DataFrame(data = reduced_data, columns = ['PC1', 'PC2'])
# principalDf['label'] = (reduced_data[:,1] > -2.8*reduced_data[:,0]-3.16)
# # principalDf['label'] = sti_df['label']
# # sti_df = sti_df[sti_df['drv_pearson'] > 0]
# sti_df['label'] = principalDf['label']
# sns.scatterplot(x="PC1", y="PC2", data=principalDf, hue='label', s=0.5, linewidth = 0, ax=ax)
# ax.plot([-2.2,-0.7],[3, -1.2], color='red')
# plt.savefig('Z:\\Christina\\FromYinan\\KCl_GT_improvement\\F9b.svg')
# plt.show()
# plt.clf()
# exit()
#
# sns.scatterplot(x="pearson", y="drv_peak", data=sti_df, hue='label', s=0.5, linewidth = 0)
# plt.savefig('Z:\\Christina\\FromYinan\\KCl_GT_improvement\\F8d.svg')
# plt.show()
# sns.distplot(sti_df['peak_ave_drv_drv'], kde=False)
# plt.show()
#
# exit()

# sti_df = sti_df[sti_df['drv_peak'] > 3]

# for sample in sti_df['sample_index'].unique():
#     sti_df_copy = sti_df.copy()
#     sti_df_copy = sti_df_copy[sti_df_copy['sample_index'] == sample]
#     sns.distplot(sti_df_copy['peak_ave_drv_peak'])
#     plt.show()
# exit()
...
# #
# # sti_df = sti_df[sti_df['drv_peak_ind'] != 22]
# mixture = GaussianMixture(n_components=1).fit(sti_df['drv_peak_ind'].values.reshape(-1, 1))
# means_hat = mixture.means_.flatten()[0]
# sds_hat = np.sqrt(mixture.covariances_).flatten()[0]
# sns.distplot(sti_df['drv_peak_ind'], bins=range(92))
# sns.distplot(mixture.sample(1000)[0].reshape(-1,1), bins=range(92))
# plt.show()
# #
# print(mixture.converged_)
# print(means_hat)
# print(sds_hat)
# print([means_hat - 1.5*sds_hat, means_hat+1.5*sds_hat])
# exit()
...
#     # else:
#     #     trace = row['trace']
#     filtered = low_pass_filter(row['trace'], sampling_freq, 0.05)
#     max_ls.append(np.max(np.gradient(filtered)[40:68]))
#     max_ind_ls.append(np.argmax(np.gradient(filtered)[40:68]))
#
# sti_df['drv_peak_interval'] =max_ls
# sti_df['drv_peak_ind_interval'] =max_ind_ls



# sti_df = sti_df.dropna().reset_index(drop=True)
# sti_df = sti_df[['ROI_index','drv_pearson', 'AUC']]
# sti_df.to_excel('Z:\\Christina\\FromYinan\\KCl_GT_improvement\\average AUC values.xlsx')
# exit()

# sns.distplot(sti_df['peak'], bins=np.arange(-3, 26, 0.1), kde=False)
# plt.savefig('Z:\\Christina\\FromYinan\\KCl_dist\\a.svg')
# plt.show()
# sns.distplot(sti_df['drv_peak_interval'], bins=np.arange(-2, 15, 0.01), kde=False)
# plt.savefig('Z:\\Christina\\FromYinan\\KCl_dist\\b.svg')
# plt.show()
# exit()

# X_all = np.concatenate((sti_df['peak'].values.reshape(-1, 1), sti_df['drv_peak_interval'].values.reshape(-1, 1)),axis=1)

# sti_df = sti_df[sti_df['peak'] > 5]
# sti_df = sti_df[sti_df['drv_peak_interval'] > 2]

# g = sns.jointplot(x="PC1", y="PC2", data=principalDf, kind="kde", color="m")
# g.set_axis_labels("$PC1$", "$PC2")
# # plt.savefig('Z:\\Christina\\FromYinan\\KCl_dist\\raw_distribution.svg')
# plt.show()
# exit()


# X = np.concatenate((sti_df['peak'].values.reshape(-1, 1), sti_df['drv_peak_interval'].values.reshape(-1, 1)),axis=1)
#
# # mixture = GaussianMixture(n_components=2).fit(np.array(max_ls).reshape(-1, 1))
# mixture = GaussianMixture(n_components=2).fit(X)
# means_hat = mixture.means_
# sds_hat = np.sqrt(mixture.covariances_)
# weights_hat = mixture.weights_
# response_ind = np.argmax([means_hat[0][0], means_hat[1][0]])
#
# print(mixture.converged_)
# print(means_hat)
# print(sds_hat)
...
# df = pd.DataFrame(mixture.sample(10000)[0], columns=['peak','drv_peak_interval'])
# g = sns.jointplot(x="peak", y="drv_peak_interval", data=df, kind="kde")
# g.set_axis_labels("$peak$", "$drv peak$")
# # plt.savefig('Z:\\Christina\\FromYinan\\KCl_dist\\fit_distribution.svg')
# plt.show()


# sns.distplot(mixture.predict_proba(X_all))
# sns.distplot(mixture.sample(10000)[0].reshape(-1,1), hist=False)
# plt.show()
# exit()
...
sti_df = sti_df.sample(20, random_state=0)
...
for row_index,row in sti_df.iterrows():
    ...
    ax.set_ylim([-1,4])
    # ax.axvline(x=peak_ind, color='green', linewidth=1)
    # ax.set_title('{}, {}, {}'.format(np.round(row['pearson'],2), np.round(row['drv_peak'],2), np.round(row['drv_pearson'],2)))
    # ax.set_title(
    #     '{}, {}, {}'.format(np.round(row['drv_peak'], 2), np.round(row['drv_pearson'], 2), np.round(row['AUC'], 1)))
    ...
    # ax.title.set_text('{}'.format(np.round(row['drv_peak'], 3)))
...
plt.savefig('Z:\\Christina\\FromYinan\\KCl_dist\\KCl_trace_peak_below_3_random_sample_20.svg')
plt.show()
